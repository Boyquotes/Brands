...
namespace Brands\Model\Base;

use \Exception;
use \PDO;
use Brands\Model\BrandQuery as ChildBrandQuery;
use Brands\Model\Map\BrandTableMap;
use Brands\Model\Thelia\Model\Product as ChildProduct;
use Brands\Model\Thelia\Model\ProductQuery;
...
abstract class Brand implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\Brands\\Model\\Map\\BrandTableMap';
    ...
    /**
     * Initializes internal state of Brands\Model\Base\Brand object.
     */
     ...
     * @return   \Brands\Model\Brand The current object (for fluent API support)
     */
    ...
    public function setNom($v)
    {
        ...
        if ($this->nom !== $v) {
            $this->nom = $v;
            $this->modifiedColumns[BrandTableMap::NOM] = true;
        }
    ...
    public function setDescription($v)
    {
        ...
        if ($this->description !== $v) {
            $this->description = $v;
            $this->modifiedColumns[BrandTableMap::DESCRIPTION] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : BrandTableMap::translateFieldName('Nom', TableMap::TYPE_PHPNAME, $indexType)];
            $this->nom = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : BrandTableMap::translateFieldName('Description', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 3; // 3 = BrandTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException("Error populating \Brands\Model\Brand object", 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(BrandTableMap::NOM)) {
            $modifiedColumns[':p' . $index++]  = 'NOM';
        }
        if ($this->isColumnModified(BrandTableMap::DESCRIPTION)) {
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(BrandTableMap::NOM)) $criteria->add(BrandTableMap::NOM, $this->nom);
        if ($this->isColumnModified(BrandTableMap::DESCRIPTION)) $criteria->add(BrandTableMap::DESCRIPTION, $this->description);
     ...
     * objects.
     *
     * @param      object $copyObj An object of \Brands\Model\Brand (or compatible) type.
     ...
     *
     * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return                 \Brands\Model\Brand Clone of current object.
     * @throws PropelException
     */
